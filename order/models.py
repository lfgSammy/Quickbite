...
class CartItem(models.Model):
    ...
    size = models.ForeignKey(MenuItemSize, on_delete=models.SET_NULL,
                             null=True, blank=True)
    rice_type = models.ForeignKey(RiceType,on_delete=models.SET_NULL,
                                  null=True, blank=True)
    ...
    def get_extras_total(self):
        ...
        shawarma_extra = sum(
            e.extra.price
            for e in self.shawarma_extra.filter(is_added=True)
        )
        return rice_extras + shawarma_extra
    ...
    def get_total(self):
        return(
            ...
            self.get_extras_total() + self.get_drinks_total()
        )
...
class OrderItemRiceExtra(models.Model):
    order_item = models.ForeignKey(OrderItem, on_delete=models.CASCADE, related_name='rice_extra')
